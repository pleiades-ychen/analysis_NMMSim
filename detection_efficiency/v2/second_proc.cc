...
// special version for detection efficiency study, v2
// 07/21/2015
...
void second_proc(const string infilename, const string outfilename)
{
  ...
  int pm;
  int cm;
  event->SetBranchAddress("TankMuon",&tm);
  event->SetBranchAddress("LeadMuon",&lm);
  event->SetBranchAddress("LeadMuonEnergy",&lmEn);
  event->SetBranchAddress("LeadNeutron",&ln);
  event->SetBranchAddress("LeadNeutronEnergy",&lnEn);
  event->SetBranchAddress("PanelMuon",&pm);
  event->SetBranchAddress("ClipMuon",&cm);
  ...
  for (int n=0;n<numEntries;n++) {
	...
	if(muonFlag)
      ...
      ev.TankMuon = tm;
      ev.LeadMuon = lm;
      ev.LeadMuonEnergy = lmEn;
      ev.LeadNeutron = ln;
      ev.LeadNeutronEnergy = lnEn;
      ev.PanelMuon = pm;
      ev.ClipMuon = cm;
    }
    //output
